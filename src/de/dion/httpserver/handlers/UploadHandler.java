...
public class UploadHandler implements HttpHandler {
    ...
    private void serveForm(HttpExchange exchange) throws IOException {
        ...
        sb.append("          <a class=\"btn\" href=\"/\" style=\"text-decoration:none;background:linear-gradient(180deg,#07220f,#05210b);padding:8px 12px;border-radius:8px;color:var(--ok);margin-left:8px\">Zurück</a>\n");
        sb.append("        </div>\n");
    ...
    private void handleUpload(HttpExchange exchange) throws IOException {
        ...
        // Verwende PushbackInputStream damit wir überlesen Bytes zurücklegen können
        try (PushbackInputStream in = new PushbackInputStream(exchange.getRequestBody(), 8192)) {
            ...
            while (!lastPartReached) {
                ...
                if (filename == null || filename.isEmpty()) {
                    // kein Datei-Teil -> verwerfen bis zur nächsten Boundary
     ...
     * Verwendet ISO_8859_1 um Probleme mit Binärdaten zu vermeiden.
     */
    ...
    /**
     * Streamt den Part direkt in die Datei bis zur nächsten Boundary.
     * Gibt true zurück, wenn die Boundary die finale Boundary ("--boundary--") war.
     *
     * Algorithmus:
     *  - Liest Byte für Byte (intern ggf. schneller durch Puffern), schreibt die meisten Bytes in FileOutputStream
     *  - Hält ein Fenster (Deque) der letzten pattern.length Bytes zurück, damit die Boundary nicht mitgeschrieben wird.
     *  - Sobald die Boundary erkannt wurde, konsumiert die restliche Boundary-Zeile und gibt zurück, ob sie final war.
     */
                ...
                for (int i = 0; i < read; i++) {
                    ...
                    if (tail.size() == patLen) {
                        ...
                        if (match) {
                            ...
                            // Nun müssen wir genau prüfen, was unmittelbar nach der Boundary kommt.
                            // Mögliche Szenarien:
                            //   --boundary\r\n    -> nächster Part
                            //   --boundary--\r\n  -> finale Boundary
                            // Wir lesen maximal 2 Bytes vor und legen überschüssige Bytes zurück.
                            int b1 = in.read();
                            int b2 = in.read();
                            ...
                            if (b1 == '-') {
                                if (b2 == '-') {
                                    ...
                                    // ungewöhnlich: b1='-' aber b2 != '-' -> zurücklegen
                                    if (b2 != -1) in.unread(b2);
                                    in.unread(b1);
                                }
                            } else if (b1 == '\r' && b2 == '\n') {
                                // normaler Fall: Grenze beendet, weiter zum nächsten Part
                            } else if (b1 == '\n') {
                                // LF-only Zeilenende; b2 gehört bereits zum nächsten Inhalt -> zurücklegen
                                if (b2 != -1) in.unread(b2);
                            } else {
                                // sonstige Bytes: legen zurück (b2 zuerst)
                                if (b2 != -1) in.unread(b2);
    ...
    /**
     * Wie streamCopyPartToFileUntilBoundary, aber verwirft die Bytes (für reine Formfelder ohne filename).
     * Gibt true zurück, wenn die Boundary die finale Boundary ("--boundary--") war.
     */
        ...
        while ((read = in.read(buffer)) != -1) {
            for (int i = 0; i < read; i++) {
                ...
                if (tail.size() == patLen) {
                    ...
                    if (match) {
                        // Boundary erkannt -> wie oben prüfen, ob final
                        int b1 = in.read();
                        int b2 = in.read();
                        boolean isFinal = false;
                        if (b1 == '-') {
